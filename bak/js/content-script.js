      ...
      case 'new':
        if (JSON.stringify(items) !== '{}') {
          ...
          chrome.storage.sync.set(items, () => {
            console.log(`save note(${id[0] == 'T' ? 'temporary' : 'new'} item)`)
      ...
      for (let each in data) {
        ...
        if (data[each]['sum'] === data[each]['num']) {
          ...
          let {
            ...
            ec: ec_id,
            po: position,
          } = data[each]['before'] ? mapping.get('before') : mapping.get('after')
          delItem = {
...
function reUpdateId(item, id) {
  ...
  nodes.forEach((node) => {
    ...
    let wn_id = pe.getAttribute('wn_id').replace(new RegExp(id[0] + '\\w+'), id)
...
function parentNode(textNode) {
  let node = textNode
  while (node.parentElement.tagName === 'SPAN' && node.parentElement.getAttribute('wn_id')) {
    ...
    if (pe.tagName === 'SPAN' && pe.getAttribute('wn_id')) {
      ...
      if (pe.childNodes.length === 1) {
        if (pe.className.split(' ').includes(type)) {
          ...
          pe.setAttribute('wn_id', wn_id)
          if (type === 'hl') {
            let style = pe.getAttribute('style').replace(/background: #\w+;/, pre_style['hl'])
            pe.setAttribute('style', style)
          }
        } else {
          // full span node
          pe.className += ' {0}'.format(type)
          pe.setAttribute('wn_id', pe.getAttribute('wn_id') + ' {0}'.format(id))
        ...
        if (pe.className.split(' ').includes(type)) {
          ...
          pe.childNodes.forEach((child) => {
            ...
            if (node === child) {
              let wn_id = wrap.getAttribute('wn_id').replace(new RegExp(id[0] + '\\w+'), id)
              wrap.setAttribute('wn_id', wn_id)
              if (type === 'hl') {
                let style = wrap.getAttribute('style').replace(/background: #\w+;/, pre_style['hl'])
          ...
          pe.childNodes.forEach((child) => {
            ...
            if (node === child) {
              ...
              wrap.setAttribute('wn_id', wrap.getAttribute('wn_id') + ' {0}'.format(id))
              wrap.setAttribute('style', wrap.getAttribute('style') + pre_style[type])
            }
            pe.parentElement.insertBefore(wrap, pe)
          })
          pe.parentElement.removeChild(pe)
...
function injectbar(range, state) {
  ...
  const btn_list = []
  const msg = ['hl', 'bold', 'italicize', 'underline', 'strike_through', 'wn_comment']
  const svg_code = [
  ...
  for (let i = 0; i < 6; i++) {
    ...
    btn.addEventListener('click', test(msg[i]))
    btn_list.push(btn)
