...
interface funcBtnProps extends ComponentProps<'button'> {
  name: string
  onClick: () => void // TODO: event parameter
  onLongPress?: () => void
  pressDuration?: number
}

export default function FuncButton(props: funcBtnProps) {
  const timeoutRef = useRef<number | undefined>(undefined)
  const [isLongPress, setIsLongPress] = useState<boolean>(false)

  const handleMouseDown = () => {
    // handle long press if with pressDuration
    if (props.pressDuration) {
      // reset flag for long press
      setIsLongPress(false)
      timeoutRef.current = window.setTimeout(() => {
        if (props.onLongPress) {
          // set true => cancel onClick event
          setIsLongPress(true)
          props.onLongPress()
        }
      }, props.pressDuration)
  ...
  return (
    <button type="button" className="wn-btn" value={props.name} onMouseDown={handleMouseDown} onMouseUp={handleMouseUp}>
      <Icon>{iconPath[props.name]}</Icon>
    </button>
  )
}
